...
const els = {
  ...
  copyAllAddressBtn: document.getElementById('copy-all-address-btn'),
...
function renderLocationDetails() {
  ...
  // Trigger automatic generation of 5 local mobile numbers & 5 local street addresses
  updateMobileNumberGeneratorUI(loc.countryCode, loc.country);
  updateStreetAddressGeneratorUI(loc);
...
function setupEventListeners() {
  ...
  // Copy All Addresses trigger
  if (els.copyAllAddressBtn) {
    els.copyAllAddressBtn.addEventListener('click', () => {
      copyAllAddresses(els.copyAllAddressBtn);
    });
  }
}
...
function parseUrlQueryParams() {
  ...
  if (searchVal) {
    ...
    renderLocationDetails();
    initLeafletMap();
    updateStreetAddressGeneratorUI(STATE.currentLocation);
  }
}
...
// ==========================================================================
// 12. Local Street Address Generator Module
// ==========================================================================

function generateSingleAddress(loc) {
  const code = (loc && loc.countryCode ? loc.countryCode : 'US').toUpperCase();
  const country = loc && loc.country && loc.country !== "N/A" ? loc.country : "United States";
  const postal = loc && loc.postalCode && loc.postalCode !== "N/A" ? loc.postalCode : "";
  const rawCity = loc && loc.city && loc.city !== "N/A" ? loc.city : "Central City";
  const rawState = loc && loc.state && loc.state !== "N/A" ? loc.state : "";
  const rawDistrict = loc && loc.district && loc.district !== "N/A" ? loc.district : "";

  // Handle parenthetical locality names e.g. "Kansat (Shibganj)"
  let city = rawCity;
  let subCity = "";
  if (rawCity.includes("(")) {
    const parts = rawCity.split("(");
    city = parts[0].trim();
    subCity = parts[1].replace(")", "").trim();
  }

  const houseNum = Math.floor(Math.random() * 180) + 1;
  const roadNum = Math.floor(Math.random() * 25) + 1;
  const aptNum = Math.floor(Math.random() * 40) + 1;
  const blockLetter = String.fromCharCode(65 + Math.floor(Math.random() * 6));

  if (code === 'DE') {
    const germanStreets = ["Hauptstraße", "Bahnhofstraße", "Schulstraße", "Gartenstraße", "Dorfstraße", "Berliner Straße", "Ringstraße", "Goethestraße", "Schillerstraße", "Lindenstraße", "Kirchstraße", "Poststraße", "Parkstraße", "Rosenstraße", "Wiesenweg"];
    const st = germanStreets[Math.floor(Math.random() * germanStreets.length)];
    const letter = Math.random() < 0.25 ? String.fromCharCode(97 + Math.floor(Math.random() * 3)) : "";
    const postCodeStr = postal ? postal : "10115";
    return `${st} ${houseNum}${letter}, ${postCodeStr} ${city}`;
  }

  if (code === 'PL') {
    const plStreets = ["Jana Pawła II", "Mickiewicza", "Słowackiego", "Kościuszki", "Sienkiewicza", "Krótka", "Polna", "Słoneczna", "Lipowa", "Ogrodowa", "Długa", "Warszawska", "Krakowska", "Kwiatowa", "Szkolna"];
    const st = plStreets[Math.floor(Math.random() * plStreets.length)];
    const hasApt = Math.random() < 0.35;
    const postCodeStr = postal ? postal : "70-525";
    return `ul. ${st} ${houseNum}${hasApt ? '/' + aptNum : ''}, ${postCodeStr} ${city}`;
  }

  if (code === 'FR') {
    const frPrefixes = ["Rue", "Rue", "Avenue", "Boulevard", "Place", "Allée"];
    const frStreets = ["Victor Hugo", "Jean Jaurès", "Charles de Gaulle", "Pasteur", "Gambetta", "de la République", "Saint-Martin", "des Fleurs", "du Général Leclerc", "Anatole France", "Voltaire", "Molière"];
    const pref = frPrefixes[Math.floor(Math.random() * frPrefixes.length)];
    const st = frStreets[Math.floor(Math.random() * frStreets.length)];
    const postCodeStr = postal ? postal : "75001";
    return `${houseNum} ${pref} ${st}, ${postCodeStr} ${city}`;
  }

  if (code === 'GB') {
    const gbStreets = ["High Street", "Station Road", "Main Street", "Church Lane", "Victoria Road", "Green Lane", "Park Road", "London Road", "Mill Lane", "Queens Road", "George Street", "King Street"];
    const st = gbStreets[Math.floor(Math.random() * gbStreets.length)];
    const hasFlat = Math.random() < 0.3;
    const flatPrefix = hasFlat ? `Flat ${aptNum}, ` : '';
    const postCodeStr = postal ? postal : "SW1A 1AA";
    return `${flatPrefix}${houseNum} ${st}, ${city} ${postCodeStr}`;
  }

  if (code === 'BD') {
    // Bangladesh address format:
    // e.g. House 18, Road 3, Kansat, Shibganj, Chapai Nawabganj 6341
    const parts = [`House ${houseNum}`];
    if (Math.random() < 0.35) {
      parts.push(`Block ${blockLetter}`);
    }
    parts.push(`Road ${roadNum}`);
    if (city && city !== "N/A") parts.push(city);
    if (subCity && subCity !== "N/A" && subCity !== city) parts.push(subCity);
    
    const distName = rawDistrict && rawDistrict !== "N/A" ? rawDistrict.replace(/ District$/i, '').trim() : "";
    if (distName && distName !== city && distName !== subCity) {
      parts.push(distName);
    }
    if (postal && postal !== "N/A") {
      parts[parts.length - 1] = `${parts[parts.length - 1]} ${postal}`;
    }
    return parts.join(", ");
  }

  if (code === 'US' || code === 'CA') {
    const usStreets = ["Main", "Oak", "Maple", "Cedar", "Pine", "Washington", "Park", "Lake", "Hill", "Sunset", "Broadway", "Elm", "Lincoln", "River", "Highland"];
    const usTypes = ["St", "Ave", "Blvd", "Dr", "Ln", "Rd", "Way", "Ct"];
    const st = usStreets[Math.floor(Math.random() * usStreets.length)];
    const type = usTypes[Math.floor(Math.random() * usTypes.length)];
    const hasApt = Math.random() < 0.4;
    const aptStr = hasApt ? `, Apt ${aptNum}${blockLetter}` : '';
    const stateStr = rawState ? `, ${rawState}` : '';
    const zipStr = postal ? ` ${postal}` : '';
    return `${houseNum * 10 + (Math.floor(Math.random()*9)+1)} ${st} ${type}${aptStr}, ${city}${stateStr}${zipStr}`;
  }

  if (code === 'ES') {
    const esPrefixes = ["Calle", "Avenida", "Paseo", "Plaza"];
    const esStreets = ["Mayor", "Gran Vía", "Alcalá", "Constitución", "San Juan", "Cervantes", "Iglesia", "Real", "Goya"];
    const pref = esPrefixes[Math.floor(Math.random() * esPrefixes.length)];
    const st = esStreets[Math.floor(Math.random() * esStreets.length)];
    return `${pref} ${st}, ${houseNum}, ${postal} ${city}`;
  }

  if (code === 'IT') {
    const itPrefixes = ["Via", "Corso", "Viale", "Piazza"];
    const itStreets = ["Roma", "Garibaldi", "Dante Alighieri", "Verdi", "Mazzini", "Vittorio Emanuele", "Cavour"];
    const pref = itPrefixes[Math.floor(Math.random() * itPrefixes.length)];
    const st = itStreets[Math.floor(Math.random() * itStreets.length)];
    return `${pref} ${st}, ${houseNum}, ${postal} ${city}`;
  }

  if (code === 'IN') {
    const inBuildings = ["Green Acres", "Sai Heights", "Sunrise Towers", "Krishna Enclave", "Gokul Apartment"];
    const bld = inBuildings[Math.floor(Math.random() * inBuildings.length)];
    const stateStr = rawState && rawState !== "N/A" ? `, ${rawState}` : '';
    return `Flat ${houseNum * 3 + 101}, ${bld}, Road No. ${roadNum}, ${city}${stateStr} ${postal}`;
  }

  // Generic fallback format
  const genStreets = ["Main", "Central", "Park", "Station", "Church", "Market", "Garden", "Victoria", "High", "River"];
  const genTypes = ["St", "Ave", "Rd", "Boulevard", "Way"];
  const st = genStreets[Math.floor(Math.random() * genStreets.length)];
  const type = genTypes[Math.floor(Math.random() * genTypes.length)];
  const stateOrDist = rawDistrict && rawDistrict !== "N/A" ? rawDistrict : (rawState && rawState !== "N/A" ? rawState : "");
  const locParts = [city];
  if (stateOrDist && stateOrDist !== city) locParts.push(stateOrDist);
  return `${houseNum * 5 + 1} ${st} ${type}, ${locParts.join(', ')} ${postal}`;
}

function generate5UniqueAddresses(loc) {
  const resultAddresses = [];
  let attempts = 0;

  while (resultAddresses.length < 5 && attempts < 200) {
    attempts++;
    const addr = generateSingleAddress(loc);
    if (!STATE.usedAddresses.has(addr)) {
      STATE.usedAddresses.add(addr);
      resultAddresses.push(addr);
    }
  }

  while (resultAddresses.length < 5) {
    const fallbackAddr = generateSingleAddress(loc);
    resultAddresses.push(fallbackAddr);
  }

  return resultAddresses;
}

function updateStreetAddressGeneratorUI(loc) {
  const container = els.addressList || document.getElementById('address-list');
  if (!container || !loc) return;

  const addresses = generate5UniqueAddresses(loc);
  STATE.currentGeneratedAddresses = addresses;

  container.innerHTML = addresses.map((addr) => `
    <div class="mobile-number-item">
      <div class="mobile-number-left">
        <i data-lucide="map-pin" class="mobile-number-icon"></i>
        <span class="mobile-number-val font-mono">${addr}</span>
        <span class="mobile-number-badge">Address</span>
      </div>
      <div class="mobile-number-actions">
        <button class="btn-copy-single btn-copy-address" data-address="${encodeURIComponent(addr)}">
          <i data-lucide="copy" class="action-icon"></i>
          <span>Copy</span>
        </button>
      </div>
  ...
  container.querySelectorAll('.btn-copy-address').forEach(btn => {
    btn.addEventListener('click', () => {
      const addrToCopy = decodeURIComponent(btn.getAttribute('data-address'));
      copySingleAddress(addrToCopy, btn);
    });
  });
}

function copySingleAddress(text, btnEl) {
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).catch(() => {});
  } else {
    const textArea = document.createElement("textarea");
...
function copyAllAddresses(btnEl) {
  if (!STATE.currentGeneratedAddresses || STATE.currentGeneratedAddresses.length === 0) return;
  const allText = STATE.currentGeneratedAddresses.join('\n');
